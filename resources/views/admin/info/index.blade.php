...
@can('customer_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-primary float-right" href="{{ route("admin.info.create") }}">
                ...
                <tbody>
                    @foreach($info as $key => $rows)
                        <tr data-entry-id="{{ $rows->id }}">
                            ...
                            <td>
                                <img width=100 src="{{ asset('images/articles/'.($rows->gambar)) }}"> 
                                {{--<img width=100 src="{{ route('admin.img.articles/',$rows->gambar) }}">--}}
                            </td>
                            <td>
                                @if($rows->is_active == 1)
                            ...
                            <td>
                                @can('customer_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.info.show', $rows->id) }}">
                                        <i class="fa fa-eye"></i> {{ trans('global.view') }}
                                    </a>
                                @endcan
                                @can('customer_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.info.edit', $rows->id) }}">
                                        <i class="fa fa-edit"></i> {{ trans('global.edit') }}
                                    </a>
                                @endcan
                                @can('customer_delete')
                                    
                                    <form action="{{ route('admin.info.destroy', $rows->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
