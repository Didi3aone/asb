...
<div class="card">
    <div class="card-header">
        {{ trans('global.update') }} {{ trans('cruds.item-category.title_singular') }}
    </div>

    <div class="card-body">
        <form class="form-material mt-4" action="{{ route("admin.item-category.update", $itemCategory->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('put')
            <div class="form-group {{ $errors->has('nama') ? 'has-error' : '' }}">
                <label for="nama">{{ trans('cruds.item-category.fields.nama') }}*</label>
                <input type="text" id="name" name="nama" class="form-control" value="{{ old('nama', $itemCategory->nama) }}">
                @if($errors->has('nama'))
